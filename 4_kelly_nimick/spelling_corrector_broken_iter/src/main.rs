...
/// Given a word, returns a HashMap containing all permutations of the word
/// resulting from the deletion of a single letter.
fn deletions(word: String) -> HashSet<String> {
...
/// Given a word, returns a HashMap containing all permutations of the word
/// resulting from the transposition of two adjacent letters.
fn transpositions(word: String) -> HashSet<String> {
    let splits = split_word(&word);
    splits.iter().filter_map(|&(front, back)| {
        if back.len() > 1 {
...
/// Given a word, returns a HashMap containing all permutations of the word
/// resulting from inserting an additional letter at any position.
fn insertions(word: String) -> HashSet<String> {
    let splits = split_word(&word);
...
/// Given a word, returns a HashMap containing all permutations of the word
/// resulting from replacing a letter at any position.
fn replacements(word: String) -> HashSet<String> {
    let splits = split_word(&word);
    let mut results = HashSet::new();
    for &(front, back) in splits.iter() {
        for c in ALPHABET.chars() {
...
/// Given a word, returns a HashMap containing only that word if the word
/// is in the dictionary. Otherwise returns an empty HashMap.
fn known(word: String, dict: &HashMap<String, usize>) -> HashSet<String> {
    let mut recognized = HashSet::new();
    if dict.contains_key(&word) {
        recognized.insert(word);
    }
    recognized
}
...
fn edits_1(word: String) -> HashSet<String> {
    deletions(word).iter().collect()
        //.chain(insertions(word).iter())
        //.chain(replacements(word).iter())
        //.chain(transpositions(word).iter())
        //.collect()
}
